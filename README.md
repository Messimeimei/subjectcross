...
相比 default 策略，`weighted` 策略具有：

* 各维度内部先归一化（不同维度采用不同方式把分数归一化到0-1区间）；
* 再按外部权重融合（单个学科在每个维度的分数 * 该维度的权重，再求和，得到这个学科最后的分数）；
* 输出更细的 detail 结构。
...
| 维度        | 归一化方法       | 理由          |
| --------- | ----------- | ----------- |
| title_abs | **Softmax** | 放大高相似度学科优势  |
| incites   | **Min–Max** | 稳定人工标签分布    |
| refs      | **Softmax** | 平滑引用极值      |
| openalex  | **Min–Max** | 保持原始比例      |
| author    | **Min–Max** | 机构数少，不宜放大差距 |
...
下面是混合用 min-max &softmax 和采用deepseek映射表 的结果：
![deepseek_map.png](./md/deepseek_map.png)
